...
Array.from(document.getElementsByClassName("songitem")).forEach(
  (element, i) => {
    if (songs[i]) {
      ...
      element.setAttribute("id", `songItem-${i}`); // Add id attribute to each song item
    }
  }
);
...
Array.from(document.getElementsByClassName("playListPlay")).forEach(
  (element, i) => {
    element.addEventListener("click", (e) => {
      ...
      updateMasterPlayButton();
      makeAllBackgrounds();
      document.getElementById(`songItem-${index}`).style.background =
        "rgba(105, 105, 170, 0.1)";
    });
  }
);

// Update play/pause button state when music ends
...
const changeSong = (increment) => {
  ...
  makeAllPlay();
  document
    .getElementById(`playListPlay-${index}`)
